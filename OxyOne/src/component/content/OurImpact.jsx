...
const OurImpact = () => {
  ...
  const impactdicription = 
    { 
      ...
      discription : " At OxyVan, we believe that small actions can lead to big changes. Through our tree planting initiative, we are not only adding greenery to our planet but also creating a lasting impact on nature and our communities. Here's how:"
    }
    ...
    <div className="container mx-auto px-4 py-8 my-10">
      ...
      <div className="mx-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8"> {
        ourImpactData.map(({title, text, image}, index)=>(
          <div key={index} className="card bg-white rounded-lg shadow-lg overflow-hidden transition-transform transform hover:scale-105 hover:shadow-xl duration-300">
            <img
            src={image}
